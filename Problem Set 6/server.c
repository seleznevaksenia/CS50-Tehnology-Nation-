...
char* indexes(const char* path)
{
 /*const char* newstr = malloc(strlen(path)+11+1);
 strncpy(path*, newstr, strlen(path));
 strncpy("/index.php", newstr + strlen(path), 10);
 newstr[strlen(path) + 10];
 strncpy("/index.html", newstr - strlen(path), 11);
 newstr[strlen(path) + 11];*/
    ...
    return NULL;*/
            ...
            char *result = malloc(strlen(path) + strlen("/index.php") + 1);
        strcpy(result,path);
        strcat(result,"/index.php");
        if (access(result, F_OK) == 0) return  result;
        char *result2 = malloc(strlen(path) + strlen("/index.html") + 1);
        strcpy(result2,path);
        strcat(result2,"/index.html");
        if(access(result2, F_OK) == 0){ free(result);return result2;
        }
        free(result);
        free(result2);
        return NULL; 
...
const char* lookup(const char* path)
{
    ...
    if (path != NULL) 
    {
        if (strcasecmp(_path, ".css") == 0)
            return "text/css";
        if (strcasecmp(_path, ".html") == 0)
            return "text/html";
        if (strcasecmp(_path, ".gif") == 0)
            return "image/gif";
        if (strcasecmp(_path, ".ico") == 0)
            return "image/x-icon";
        if (strcasecmp(_path, ".jpg") == 0)
            return "image/jpeg";
        if (strcasecmp(_path, ".js") == 0)
            return "text/javascript";
        if (strcasecmp(_path, ".png") == 0)
            return "image/png";
        if (strcasecmp(_path, ".php") == 0)
            return "text/x-php";
    }
        return NULL;
...
bool parse(const char* line, char* abs_path, char* query)
{
   char linecopy[strlen(line) + 1];
        ...
        //разбиваем строку на части разделителем пробел
        methodcopy = strtok(linecopy, " ");
        requesttargetcopy = strtok(NULL, " ");
        httpversioncopy = strtok(NULL, "\r\n");
    
        //копируем метод запрос и версию в меременную типа массива
        ...
        //если запрос не начинаеться с слеша то ошибка
        if (strncmp(requesttarget, "/", 1) != 0)
        {
            error(501);
        }
        // если  request-target содержит кавычку,то  error 400  найти символ в строке
        if (strchr(requesttarget, '"') != NULL)
        {
            error(400);
        }
        // проверка пробелов после GET
        if (line[4] == ' ')
        {
            error(400);
            return false;       
        }
        // проверка пробелов до HTTP
        char* http_version = httpversion;
        // в переменную пишем указатель на HTTP
        char* TwoSpasesHTTP = strchr(http_version, ' ');
        if (TwoSpasesHTTP != NULL)
        {
            error(400);
            return false;   
        }
        ...
        if (strcmp(httpversion, "HTTP/1.1") != 0)
        {
         ...
         //Проверяем absolute-path [ "?" query ]
        char* abs_pathcopy;
        char* querycopy;
        abs_pathcopy = strtok(requesttarget, "?");
        querycopy = strtok(NULL, "\0");
    
        // /hello.php?q=Alice если есть запрос
        if (querycopy != NULL)
        {
